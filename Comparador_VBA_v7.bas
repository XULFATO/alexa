Attribute VB_Name = "Comparador_VBA"
Option Explicit

' ============================================================
'  COMPARADOR DE EXCELS  v7
'  Unico cambio respecto a v6:
'    - Constante UMBRAL_TOPE
'    - Subs BotonImportar1 y BotonImportar2 con logica posicional
...
' ============================================================
'  A PARTIR DE AQUI: CODIGO v6 IDENTICO, SIN NINGUN CAMBIO
' ============================================================
...
Sub CompararHojas()
    ...
    For col = 1 To lastCol1
        If LCase(Trim(CStr(ws1.Cells(1, col).Value))) = "* employee id" Or _
           LCase(Trim(CStr(ws1.Cells(1, col).Value))) = "employee id" Then
            colEmp1 = col
            Exit For
        End If
    Next col
    For col = 1 To lastCol2
        If LCase(Trim(CStr(ws2.Cells(1, col).Value))) = "* employee id" Or _
           LCase(Trim(CStr(ws2.Cells(1, col).Value))) = "employee id" Then
            colEmp2 = col
            Exit For
        End If
    Next col
    ...
    For k = 1 To nAll
        ...
        For e = 1 To nIdx2
            If idx2Keys(e) = empID Then
                r2 = idx2Rows(e): Exit For
            End If
        Next e

        difFila = False
        If r1 = 0 Then
            estado = "SOLO EN V2": difFila = True
        ElseIf r2 = 0 Then
            estado = "SOLO EN V1": difFila = True
        Else
            estado = "NO"
        End If
